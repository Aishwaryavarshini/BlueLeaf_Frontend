...
export const aiService = {
  ...
  async sendMessageStream(message: string, history: any[], systemInstruction: string) {
    // Mock streaming response with dual explanation format
    const mockResponse = `I understand your question about "${message}". Here is a simple explanation for you. --- This topic involves complex interactions that can be formally described as follows. In a real scenario, this would be generated by an AI model based on the syllabus and your preferences.`;
  ...
  async generateTTS(text: string, voice: string): Promise<string | null> {
    await new Promise(resolve => setTimeout(resolve, 500));
    // Return null as we don't have real audio assets in mock mode
    return null; 
  },
      ...
      sendRealtimeInput: (input: any) => {
        ...
        setTimeout(() => {
          ...
          if (callbacks.onmessage) {
            callbacks.onmessage({
              serverContent: {
                inputTranscription: { text: "Can you explain this further?" }
              }
            });
          }
        }, 1000);
        ...
        setTimeout(() => {
          ...
          if (callbacks.onmessage) {
            callbacks.onmessage({
              serverContent: {
                outputTranscription: { text: "Certainly! Let's dive deeper into the core principles. --- We shall now elaborate on the underlying theoretical constructs." },
                turnComplete: true
              }
            });
          }
